...
import static com.duprasville.limiters.treefill.TreeFillMath.clusterRemainingTriggersPerRound;
import static com.duprasville.limiters.treefill.TreeFillMath.nodeRemainingTriggersPerRound;
...
class TreeFillMathTest {
    ...
    void testClusterRemainingTriggersPerRound() {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            long[] clusterRemainingPerRound = clusterRemainingTriggersPerRound(clusterPermits, clusterSize);

            assertThat(clusterRemainingPerRound[0], is(equalTo(clusterPermits)));

            if (clusterPermits <= clusterSize) {
                assertThat("when permits < cluster size, remaining is a single element array with permits",
                        clusterRemainingPerRound.length, is(1));
            } else {
                assertThat(
            ...
            for (int clusterNode = 0; clusterNode < clusterSize; clusterNode++) {
                allNodesRemainingPerRound[clusterNode] = nodeRemainingTriggersPerRound(clusterPermits, clusterNode, clusterSize);
            }
