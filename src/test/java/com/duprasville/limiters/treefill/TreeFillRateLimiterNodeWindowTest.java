...
public class TreeFillRateLimiterNodeWindowTest extends TreeFillRateLimiterTestBase {
    ...
    @Test
    void testMultiAcquireWithOneNodeAndFivePermits() {
    ...
    void testRaceBetweenPastDetectAndRoundIncrement() {
        ...
        for (Message message : messagesSent) {
            assertEquals(Message.MessageType.Detect, message.type);
        }
    }
    ...
    void testRaceBetweenIncrementRoundAndDetectFromTheFuture() {
        ...
        Message message = mockMessageSender.messagesSent.get(0);
        assertEquals(Message.MessageType.Acquire, message.type);
